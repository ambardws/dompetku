    ...
    <div class="p-6">
      ...
      <div v-if="summary" class="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div class="stat-item group">
          ...
          <p class="text-2xl font-bold text-emerald-600">Rp {{ formatAmount(summary.totalIncome) }}</p>
        </div>
        ...
        <div class="stat-item group">
          ...
          <p class="text-2xl font-bold text-rose-600">Rp {{ formatAmount(summary.totalExpense) }}</p>
        </div>
        ...
        <div class="stat-item group">
          ...
          <p
            ...
            :class="summary.balance >= 0 ? 'text-primary-600' : 'text-rose-600'"
          >
      ...
      <div v-if="summary && (summary.topExpenseCategory || summary.topIncomeCategory)" class="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <!-- Top Expense -->
        <div v-if="summary.topExpenseCategory" class="top-category bg-white border border-gray-200 rounded-xl p-5">
          <div class="flex items-center gap-2 mb-3">
            <div class="w-6 h-6 bg-rose-50 rounded-lg flex items-center justify-center border border-rose-100">
          ...
          <div class="flex items-center gap-3">
            <div
              class="w-14 h-14 rounded-xl flex items-center justify-center text-2xl bg-gray-50 border border-gray-100"
        ...
        <!-- Top Income -->
        <div v-if="summary.topIncomeCategory" class="top-category bg-white border border-gray-200 rounded-xl p-5">
          <div class="flex items-center gap-2 mb-3">
            <div class="w-6 h-6 bg-emerald-50 rounded-lg flex items-center justify-center border border-emerald-100">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-6">
        <!-- Expense Chart -->
        <div v-if="summary && summary.expenseByCategory.length > 0" class="bg-gray-50 rounded-2xl p-5">
        ...
        <!-- Income Chart -->
        <div v-if="summary && summary.incomeByCategory.length > 0" class="bg-gray-50 rounded-2xl p-5">
          <h4 class="text-sm font-bold text-gray-800 mb-4 flex items-center gap-2">
      ...
      >
        ...
        <p class="text-gray-700 font-semibold text-lg mb-1">Belum ada data transaksi untuk periode ini</p>
        <p class="text-gray-500 text-sm">Mulai tambahkan transaksi untuk melihat analisis</p>
      </div>
    </div>
  </div>
</template>
...
.stat-item {
  @apply p-5 bg-white rounded-xl border border-gray-200;
  transition: all 0.2s ease;
}

.stat-item:hover {
  @apply shadow-sm border-gray-300 -translate-y-0.5;
}
